...
#friedman.test(y = EO.Master.2$'Average Time Played (Sec)' , groups = EO.Master.2$'Group #'))
```
...
```{r}
Rein.Master.2 <- read_excel('Play Exp 3 (Reinforcer) Data MASTER2.xlsx', sheet = 'Master Reinforcer')
#friedman.test(y = Rein.Master.2$`Time Played (Sec)`, groups = Rein.Master.2$`Group #`, blocks = Rein.Master.2$`Sample #`)
```
