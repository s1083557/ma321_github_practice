...
wilcox.test(x = EO.MASTER$`0 min` , y = EO.MASTER$'30min' , paired = TRUE, conf.int = TRUE) # A post hoc test comparing 0 min and 30 min pairwise. 
wilcox.test(x = EO.MASTER$`0 min` , y = EO.MASTER$'120min' , paired = TRUE, conf.int = TRUE) # A post hoc test comparing 0 min and 120 min pairwise. 
wilcox.test(x = EO.MASTER$`0 min` , y = EO.MASTER$`>360min` , paired = TRUE, conf.int = TRUE) # A post hoc test comparing 0 min and >360 min pairwise. 
wilcox.test(x = EO.MASTER$'30min' , y = EO.MASTER$'120min' , paired = TRUE, conf.int = TRUE) # A post hoc test comparing 30 min and 120 min pairwise. 
wilcox.test(x = EO.MASTER$'30min' , y = EO.MASTER$`>360min` , paired = TRUE, conf.int = TRUE) # A post hoc test comparing 30 min and >360 min pairwise. 
wilcox.test(x = EO.MASTER$'120min' , y = EO.MASTER$`>360min` , paired = TRUE, conf.int = TRUE) # A post hoc test comparing 120 min and 360 min pairwise. 

```
When doing the friedman's test of the EO Dataset, with a p-value of 0.9343 we fail to reject the null hypothesis that the means across the four groups are equal. This is an inconclusive result. After doing the wilcoxon signed rank test for post-hoc analysis we see that all confidence intervals cross zero indicating that it is not statistically significant. Not enough evidence is available to suggest that the medians are not equal.
...
```
The kruskall-wallis test for the EO dataset also indicates that the there is not enough evidence to reject the null hypothesis of equal means across the four groups. 
...
```
The above post hocs for the kruskall-wallis test show that there isn't enough evidence to reject the null hypothesis. This makes sense because the conclusion of the k-w test is to not reject the null hypothesis, essentially, the post-hocs are indicative of that conlusion.
